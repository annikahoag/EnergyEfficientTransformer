...
def optimize_model(epoch, model, data, dataO, optimizer, scheduler,config, training=True):
    ...
    if 'TransNAS_TSAD' in model.name:
        ...
        data_tensor = torch.DoubleTensor(data.type(torch.DoubleTensor))
        ...
        if training:
            ...
            for batch_data, _ in data_loader:
                ...
                target_element = target_element.to(device)
            ...
            #print("\nDevice: ", torch.cuda.get_device_name(0))#next(model.parameters()).device)
            ...
            #Converting back to CPU to use numpy
